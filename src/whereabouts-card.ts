...
class WhereaboutsCard extends LitElement {
    ...
    getZoneEntityIdByFriendlyName(hass: any, friendlyName: string): string | undefined {
        return Object.keys(hass.states).find((eid: string) => eid.startsWith('zone.') && hass.states[eid]?.attributes?.friendly_name === friendlyName);
    }
            ...
            <ha-card>
                ...
                <div>
                    ${this.persons.map((person) => {
                        ...
                        const personState = entity.state; // friendly name of the zone

                        // Find matching zone entity ID using the friendly name
                        const zoneEntityId = this.getZoneEntityIdByFriendlyName(this.hass, personState);
                        const zoneFriendlyName = personState;
                        let usedPreposition = this.default_preposition;
                        let showPreposition = true;
                        let zoneNameOverride: string | undefined;

                        // When matching zone_groups, compare both entity ID and friendly name
                        if (Array.isArray(this.zone_groups)) {
                            for (const group of this.zone_groups) {
                                if ((zoneEntityId && group.zones.includes(zoneEntityId)) || group.zones.includes(zoneFriendlyName)) {
                        ...
                        const zoneDisplay = zoneNameOverride ?? zoneFriendlyName ?? personState;

                        return html` <div>${name} ${this.default_verb} ${showPreposition ? usedPreposition + ' ' : ''} ${zoneDisplay}</div> `;
                    })}
                </div>
            </ha-card>
        `;
